...
class XFVRPSwapSpec extends Specification {

	def service = new XFVRPSwap();
	...
	def "Search single depot - Find improve"() {
		...
		sol = new Solution()
		sol.setGiantRoute([nd, n[2], n[4], nd, nd, n[3], n[5], nd] as Node[])
		...
		then:
		impList.size() > 0
		impList.stream().filter({f -> f[0] == 1 && f[1] == 6}).count() == 1
		impList.stream().filter({f -> f[0] == 2 && f[1] == 5}).count() == 1
		Math.abs(impList.stream().filter({f -> f[0] == 1 && f[1] == 6}).collect(Collectors.toList()).get(0)[2] - 8.385) < 0.001f
		Math.abs(impList.stream().filter({f -> f[0] == 2 && f[1] == 5}).collect(Collectors.toList()).get(0)[2] - 8.385) < 0.001f
	}
	...
	def "Search multi depot - Find improve"() {
		...
		Math.abs(impList.stream().filter({f -> f[0] == 2 && f[1] == 4}).collect(Collectors.toList()).get(0)[2] - 14.094) < 0.001f
	}
