...
public class ReportBuilder {
    ...
    public Report getReport(XFVRPSolution solution) {
        ...
        Node[] giantRoute = solution.getSolution().getGiantRoute();

        // Wenn die Giant Route Kunden enthält
        if(findNextCustomer(giantRoute, 0) != -1) {
            final float[][] routeDepotServiceMap = createRouteDepotServiceMap(giantRoute, model.getParameter());
            ...
            for (int i = 1; i < giantRoute.length; i++) {
                ...
                if(currNode.getSiteType() == SiteType.DEPOT) {
                    ...
                    eval(lastNode, lastDepot, tRep, routeDepotServiceMap, model);
                    ...
                    eval(lastNode, currNode, tRep, null, model);
     ...
     */
    private void eval(Node lastNode, Node currNode, RouteReport tRep, float[][] routeDepotServiceMap, XFVRPModel model) {
    ...
    private float[][] createRouteDepotServiceMap(Node[] giantRoute, XFVRPParameter parameter) {
